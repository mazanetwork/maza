...
Maza now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
 ...
 * finalized budget voting - mazanodes that agree with the finalization will vote on that budget
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ MAZA, _12_ times over the course of a year totaling _24000_ MAZA.

**Warning: if you change any fields within this command, the collateral transaction will become invalid.**

Format: ```mngovernance prepare proposal-name url payment-count block-start maza-address monthly-payment-maza```
...
Format: ```mngovernance submit proposal-name url payment-count block-start maza-address monthly-payment-maza fee-tx```
...
If everything looks correct, you can ask for votes from other mazanodes. To vote on a proposal, load a wallet with _mazanode.conf_ file. You do not need to access your cold wallet to vote for proposals.
...
After you get enough votes, execute ```mngovernance projection``` to see if you made it into the budget. If you the budget was finalized at this moment which proposals would be in it. Note: Proposals must be active at least 1 day on the network and receive 10% of the mazanode network in yes votes in order to qualify (E.g. if there is 3500 mazanodes, you will need 350 yes votes.)
...
When block ```1000000``` is reached you'll receive a payment for ```1200``` MAZA to ```y6R9oN12KnB9zydzTLc3LikD9cCjjQzYG7```.
...
The following RPC commands are supported:
 ...
 - mngovernance "command"... ( "passphrase" )
  ...
  - vote               - Vote on a proposal by single mazanode (using maza.conf setup)
  - vote-many          - Vote on a proposal by all mazanodes (using mazanode.conf setup)
