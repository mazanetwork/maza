...
- saction for Darksend. These are special transactions that require a signature that only the mazanodes can create.
...
- Clients remember which mazanodes they've connected to in the past and won't use them against.
...
- Split up darksend.cpp into mazanode.cpp, activemazanode.cpp and instantx.cpp
- Added modular ProcessMessages for Darksend, Mazanodes and InstantX
- Client can now join sessions with any other users

0.10.15.20/21 Release notes
---------------------------

- added bloom filters for dsee/dseep broadcasts, moved expensive dsee search, mazanode vote caching
...
- Fixed mazanode payment check for out-of-order blocks
...
- mazanode only take newest dsee now


0.10.15.15 Release notes
------------------------

- fixed fragmentation issue, allow mazanodes to update pubkey2
...
- Fixed the stuck blockchain (due to mazanode payments)
- Improves collateral creation for those getting "invalid collateral"
- Minimum confirmation requirements before mazanode payment.
  Must have more confirmations then there are mazanodes.
...
- Changes the mazanode payments keys, you'll only recieve mazanode
...
- I've made some huge progress with enforcement. Each mazanode will
...
- Fixed some issues with mazanode payments syncing
...
- Fixed some issues with the way the mazanode payment list
  was being handled
- Mazanode confirmation error now displays 200 required instead of 6
...
- security update: This update simply verifies that the signature was
  not forged and that the mazanodes entering the list are authentic.
...
- mazanode constants for communication are much easier to read now
...
- Improved handling of enforcement for bad mazanode lists
- Removed old mazanode override code
...
- Darksend UI is disabled for mazanodes now and titlebar says "[mazanode]"
- Improvement for dealing with splitting large inputs
- Protocol version bump to kick old mazanodes off
...
- Added a spork for enforcing mazanode payments (this will ensure misconfigured
...
- This update deals with freezing, slow wallets, slow load times and the
  ...
  I've modified the queuing system, so users wait in a mazanode queue without
...
- removed matching mazanode debugging messages, that's not really an error
- Client now prioritises sending denominated funds back into Darksend. This will
  ...
  configuration.
- fixed a bug where mazanodes send failed transactions
...
- Darksend now checks diskspace before sending anything to a mazanode.
...
- Disabled collateral charging for now. We'll work on this after RC4 is
  released and update the mazanode network after it's working properly. It's
...
- I've switched up the way the mazanode network works.
    1.) Users now will join a random mazanode (1 of the entire list, just
        completely randomly)
    2.) Upon joining if it's the first user, the mazanode will propagate
...
- Fixed some bugs with connecting to the correct mazanodes
...
- Added session IDs for mazanode communication. Clients were getting
  confused when they got messages about other sessions (sometimes happened when
  they all jumped on the same mazanode at once)
- Added a pre-session state where the client will query a random mazanode
...
- Added code to randomly use the top 20 mazanodes, this can dynamically be
...
- After successful transactions clients will now automatically attempt another
  session on a random mazanode, then repeat until they get any kind of error
  or run out of funds that need to be processed.
- Fixed a change address reuse issue
- Fixed an issue with the compatible join algorithm (Mazanodes will only join
...
- Fixed an issue where clients weren't connected to the correct mazanode
- Fixed mazanode relay issues
...
- This version automatically resets the mazanode state machine after a short
  period of inactivity.
- Updated protocol version to kick old mazanodes off
...
- Found a communication mix up where clients would see messages from the wrong
  mazanode and think it was theirs, also causing collateral fees
...
- incremented protocol version to force mazanode updates
...
- Client will only submit 1 transaction into the pool fixing possible
  anonymity issues
- Mazanodes will only merge compatible transactions using the same
...
- Added possible fix for mazanode list syncing
...
- Mazanodes now reset themselves when they give "entries is full".
...
on the same mazanode for testing purposes:
/darkcoin-qt -datadir=/home/user/.darkcoin -listen=0 -darksendrounds=8 -usemazanode="192.168.56.102:19999"
/darkcoin-qt -datadir=/home/user/.darkcoin2 -listen=0 -darksendrounds=8 -usemazanode="192.168.56.102:19999"

and even disable darksend auto-denom if wanted:
/darkcoin-qt -datadir=/home/user/.darkcoin -listen=0 -darksendrounds=8 -usemazanode="192.168.56.102:19999" -disabledarksend=1
...
- Mazanodes should change every block now (missed a mod=10 last time)
...
- Mazanodes should change every block now
...
- usemazanode option, will override active mazanode (only in RC, just for testing)
...
- Fixed payout issues (mazanode consessus was paying out to vout(0) by default)
- Improved DarksendInput add entry verification. Mazanodes will now reject
...
- Fixed a signing bug with the mazanode voting system causing a bunch of issues
- Updated unit tests
