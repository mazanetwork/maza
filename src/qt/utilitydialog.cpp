...
All the Maza in your wallet is comprised of different \"inputs\" which you can think of as separate, discrete coins.<br> \
...
These denominations are 0.01 MAZA, 0.1 MAZA, 1 MAZA and 10 MAZA -- sort of like the paper money you use every day.</li> \
<li>Your wallet then sends requests to specially configured software nodes on the network, called \"mazanodes.\" \
These mazanodes are informed then that you are interested in mixing a certain denomination. \
No identifiable information is sent to the mazanodes, so they never know \"who\" you are.</li> \
...
The mazanode mixes up the inputs and instructs all three users' wallets to pay the now-transformed input back to themselves. \
...
For more info see <a href=\"https://mazacoin.atlassian.net/wiki/display/DOC/PrivateSend\">https://mazacoin.atlassian.net/wiki/display/DOC/PrivateSend</a> \
        "));
        ui->aboutMessage->setWordWrap(true);
        ui->helpMessage->setVisible(false);
        ui->aboutLogo->setVisible(false);
