...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in maza.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in maza.qrc)
...
/** Class encapsulating Maza Network startup and shutdown.
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. Maza Network can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    /// 6. Determine availability of data directory and parse maza.conf
...
#ifdef ENABLE_WALLET
    /// 7a. parse mazanode.conf
    std::string strErr;
    if(!mazanodeConfig.read(strErr)) {
        QMessageBox::critical(0, QObject::tr("Maza Network"),
                              QObject::tr("Error reading mazanode configuration file: %1").arg(strErr.c_str()));
        return EXIT_FAILURE;
    }
    ...
    // maza: links repeatedly have their payment requests routed to this process:
